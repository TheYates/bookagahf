...
export default function DoctorSettingsPage() {
  ...
  const toggle = async () => {
    ...
    const { error } = await supabaseBrowserClient
      .from("doctor_settings")
      .upsert({ doctor_id: userId, is_available: next, updated_at: new Date().toISOString() })
    ...
    setIsAvailable(next)
    setMessage({
      text: next ? "You are now available for bookings." : "You are now unavailable. Clients cannot book you.",
      ok: next,
    })
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Manage your availability and preferences.</p>
      </div>
      ...
      <div className="rounded-xl border bg-background p-6 shadow-sm">
          ...
          {loading ? (
            ...
            <button
              ...
              className={cn(
                ...
                isAvailable ? "border-green-500 bg-green-500" : "border-muted-foreground bg-muted",
              )}
        ...
        {!loading && (
          <div className={cn(
            ...
            isAvailable ? "bg-green-50 text-green-700" : "bg-muted text-muted-foreground",
          )}>
            <Power className="h-4 w-4" />
            {isAvailable ? "Accepting new appointments" : "Not accepting new appointments"}
          </div>
        )}
