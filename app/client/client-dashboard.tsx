...
export default function ClientDashboardPage({
  ...
  const [userName, setUserName] = React.useState(serverUserName || "")
  ...
  React.useEffect(() => {
    const init = async () => {
      ...
      if (user) {
        // If server already gave us a name, only overwrite if profiles
        // has a more reliable value that differs from user_metadata
        if (!serverUserName) {
